...
def main() :
    ...
    model.coef_ = np.zeros(2)         # [ 0.  0.]
    cost = model.cost(train_data.X, train_data.y)
    print('Cost for part d =', cost)
    print()
    #print(model.predict(train_data.X))
    
    print ('Testing GD for 10^-4,10^−3,10^−2,0.0407')
    # part d/2
    for elem in [0.0001, 0.001, 0.01, 0.0407]:
        time_ini = time.time()      # get initial time
        print('When η =', elem) 
        model.fit_GD(train_data.X, train_data.y, elem, verbose=False)
        print('Time (duration):', time.time()-time_ini)
        print('Cost:', model.cost(train_data.X, train_data.y))
        print('Coefficient vector θ:', model.coef_)                        
        print()
    
    # part e
    print('Testing closed form...')
    time_ini = time.time()
    ...
    ms = range(11)
    plt.plot(ms, trainErrs, 'bo-', \
             ms, testErrs, 'ro-')
    blue_circle = mpl.lines.Line2D([], [], color='b', marker='o', label='training error')
    red_circle = mpl.lines.Line2D([], [], color='r', marker='o', label='test error')
